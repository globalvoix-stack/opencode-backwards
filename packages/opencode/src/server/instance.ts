...
export const InstanceRoutes = (upgrade: UpgradeWebSocket, app: Hono = new Hono()) =>
  app
    ...
    .all("/*", async (c) => {
      ...
      const webRoot = Flag.OPENCODE_WEB_ROOT
      ...
      if (embeddedWebUI) {
        ...
        if (!full.startsWith(root)) return c.json({ error: "Not Found" }, 404)
        const found = (await fs.exists(full)) && (await fs.stat(full)).isFile()
        const target = found ? full : resolve(root, "index.html")
        if (!(await fs.exists(target))) return c.json({ error: "Not Found" }, 404)
        const mime = getMimeType(target) ?? "text/plain"
        c.header("Content-Type", mime)
        if (mime.startsWith("text/html")) {
          c.header("Content-Security-Policy", DEFAULT_CSP)
        }
